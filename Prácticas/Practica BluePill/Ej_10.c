// Implementar en C de bajo nivel una función delay a la que se le pase como argumento el
// tiempo de retardo en ms, utilizando el SysTick y la interrupción asociada a él.
...
int main(void)
{
    ...
    // Arranca apagado
	GPIOC_ODR |= (1u << 13);
    SYSTICK_RVR = 1000 - 1; // El SysTick cuenta 1000 ticks y genera una interrupción.
...
// Reloj! Su único trabajo es incrementar la variable global (systick_counter++) 1000 veces por segundo, actualizando la "hora" actual en la variable systick_counter
void SysTick_Handler(void){
    systick_counter++; 
}
