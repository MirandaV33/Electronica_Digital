/*
Implementar un código C de bajo nivel (sin utilizar las funciones de biblioteca) que genere una
salida cuadrada por uno de los GPIO utilizando DMA. Verificar utilizando un osciloscopio.
*/
...
// Señal cuadrada
void init_square_wave(volatile uint32_t arr[256])
{
	for (int i = 0; i < 256; i++) {
		if (i < 256 / 2) {
			arr[i] = 0x00002000u;  // nivel alto
		} else {
			arr[i] = 0x00000000u;  // nivel bajo
		}
	}
}
...
int main(void)
{
	init_square_wave(data);

	// Configuracion del RELOJ
	DEVMAP->RCC.REGs.CR   |= (1 << 16);	// habilita HSE (clk externo de alta velocidad)				// Enable HSE
	while (!(DEVMAP->RCC.REGs.CR & (1 << 17)));	// espera a que hse este listo
	...
	// Configuracion Perifericos (DMA y Timer)
        // Se encienden los perifericos que se van a usar
