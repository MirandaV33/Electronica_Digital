...
Es un método que permite transferir datos entre periféricos y memoria sin la intervención del
procesador, liberando así recursos de la CPU para otras tareas y mejorando la eficiencia
del sistema

QUÉ ES DEVMAP?
Es una estructura de datos que representa el mapa de memoria de los periféricos del microcontrolador.
En vez de hacerlo con punteros a direcciones específicas, se utiliza una estructura organizada que facilita
Ahora, en lugar de usar un #define para cada registro, puedes acceder a ellos de forma lógica, como si fueran carpetas
*/
...
int main(void)
{
	...
	// Configuracion del reloj PERO no usando punteros, sino con la estructura DEVMAP
	// PCLK code
	...
	// DMA code
	//Habilitamos relojes de perifericos
	...
	//Configuramos GPIOC como salida
...
//	DEVMAP->GPIOs[GPIOC].REGs.ODR ^= -1;
	...
	// Configuramos el canal 2 del DMA1
	...
	// Configure DMA1 Channel2 CCR register
